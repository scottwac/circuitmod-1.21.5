    ...
    private void drawStatusInfo(DrawContext context) {
        ...
        if (handler.hasValidPartner()) {
            if (handler.isScanning()) {
                ...
                context.drawText(textRenderer, scanText, baseX + 8, baseY + 32, 0xFFFF00, false); // Yellow
                
                // Draw progress bar
                drawProgressBar(context, baseX + 8, baseY + 44, 100, 8, handler.getScanProgress());
            } else {
                Text readyText = Text.literal("Ready - Insert Blueprint");
                context.drawText(textRenderer, readyText, baseX + 8, baseY + 32, 0x00FF00, false); // Green
            }
        } else {
            Text instructionText = Text.literal("Place partner desk to connect");
            context.drawText(textRenderer, instructionText, baseX + 8, baseY + 32, 0xAAAAAA, false); // Gray
        }
    }
